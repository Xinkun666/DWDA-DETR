...
from models.qna import FusedKQnA
...
class TransformerStage(nn.Module):
    ...
    def __init__(self, fmap_size, window_size, ns_per_pt,
        ...
        for i in range(depths):
            ...
            elif self.stage_spec[i] == 'DW':
                self.attns.append(
                    DeformableWindowAttention(size=fmap_size[0],dim=dim_embed,num_heads=heads)
                )
            else:
...
class DAT(nn.Module):
    ...
    def __init__(self,
                 ...
                 # 控制Transformer结构
                 expansion=4,dim_stem=32, dims=[48, 96, 192, 384], depths=[2, 2, 4, 1], heads=[2, 4, 8, 16], heads_q=[4, 8, 16, 32],
                 # 控制局部注意力和dropout率
                 window_sizes=[7, 7, 7, 7],drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.0,
                 #控制不同阶段特性，步长、偏移范围、每个阶段结构
                 strides=[1,1,1,1],offset_range_factor=[1, 2, 3, 4],stage_spec=[['DW','D'], ['DW','D'], ['DW', 'D','DW','D'], ['D']],
                 # 进一步细化模型特征提取方式和学习策略
                 groups=[1, 1, 3, 6],
    ...
    def forward(self, x):
        ...
        # total_params = sum(p.numel() for p in self.patch_proj.parameters())
        # print(f"Patch_proj Parameters Number: {total_params}")
