...
class DAttentionBaseline(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"DAttentionBaseline Parameters Number: {total_params}")
        ...
        # offset_numpy = offset.cpu().detach().numpy()
        # np.save(f'./output/offset/offset{Hk}.npy',offset_numpy)
...
class DeformableWindowAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        kv_mask = kv_mask.unsqueeze(1).unsqueeze(-2).repeat(1,self.heads,1,1,1)
        attn_mask_w = attn_mask_w.unsqueeze(1).unsqueeze(-2).repeat(1,self.heads,1,1,1)
        attn_mask_w = attn_mask_w.to(x.device)
        # v_w = v_w * kv_mask
